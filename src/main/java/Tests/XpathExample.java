...
public class XpathExample extends BaseTest{
	...
	public void xpathExample2() {
		...
		driver.findElement
		  (By.xpath("//span[@class='user_name']")).click();
		...
		//table//span[not(contains(text(), 'Order')) and not(contains(text(), 'Status')) and not(contains(@class, 'woocommerce')) and not(contains(text(), 'Total')) and not(contains(text(), 'Actions')) ]
		//not()
		...
		//index
		WebElement statusTableHeader = driver.findElement(By.xpath("(//span[@class='nobr'])[3]"));
		jse.executeScript("arguments[0].setAttribute('style', 'background:yellow ;border:10px solid green')", 
				statusTableHeader);
